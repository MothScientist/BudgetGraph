...
CREATE TABLE IF NOT EXISTS "budget_graph"."groups" (
    ...
    "owner"        bigint      NOT NULL UNIQUE CHECK("owner" > 0 AND "owner" < POWER(10, 12)),
    "token"        varchar(32) NOT NULL UNIQUE CHECK(LENGTH("token") = 32),
    "users_number" smallint        NULL        CHECK(("users_number" > 0 AND "users_number" <= 20) OR "users_number" IS NULL) -- number of participants in the group
);
CREATE TABLE IF NOT EXISTS "budget_graph"."users" (
    "telegram_id" bigint      PRIMARY KEY        CHECK("telegram_id" > 0 AND "telegram_id" < POWER(10, 12)),
    "username"    varchar(20) NOT NULL    UNIQUE CHECK(LENGTH("username") >= 3 LENGTH("username") <= 20),
...
CREATE TABLE IF NOT EXISTS "budget_graph"."monetary_transactions" (
    ...
    "username"       varchar(20) NOT NULL CHECK(LENGTH("username") >= 3 AND LENGTH("username") <= 20),
...
CREATE TABLE IF NOT EXISTS "budget_graph"."user_languages_telegram" (
    "telegram_id"  bigint     NOT NULL UNIQUE CHECK("telegram_id" > 0 AND "telegram_id" < POWER(10, 12)),
...
CREATE TABLE IF NOT EXISTS "budget_graph"."premium_users" (
    "telegram_id" bigint  NOT NULL CHECK("telegram_id" > 0 AND "telegram_id" < POWER(10, 12)),
